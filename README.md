...
네 분의 사진은 이미 들어가 있습니다. 교체하려면 `assets/img/` 에 같은 파일명으로
덮어쓰기만 하면 됩니다. HTML 수정은 필요 없습니다.
...
- 현재 파일은 원본을 정사각형으로 잘라 400x400 JPG 로 저장한 것입니다 (각 14~20KB)
- 원형으로 표시되며, 정사각형이 아닌 이미지는 `object-fit: cover` 로 채워집니다
